...
TODO (rozwinąć): Pakiet usług jest definiowany na węźle lokalnym zleceniodawcy. Pakiet usług predefiniuje listę konsultantów.
...
\begin{enumerate}
  \item Transfer plików związanych ze zleceniem następuje:
	\begin{enumerate} 
		\item bezpośrednio po zdefiniowaniu zlecenia jeśli nie są aktualnie transferowane inne zlecenia
		\item 
  ...
  \begin{enumerate} TODO przenieść do opisu interfejsu
    \item Bieżący stan transferu w postaci ilości serii/łączna liczba serii i ilości obrazów w każdej serii.
...
10. Routing badań jest wykonywany w oparciu o następujące parametry:
		\begin{enumerate}
		\item dostępności węzła - czy węzeł docelowy jest dostępny, czy nie (dostępność jest badana w sposób aktywny za pomocą zapytań HTTP)
		\item czasy obsługi zleceń.
		\item preferencji zlecającego.
		\item statystyki z list i opisów powinny być zachowywane i archiwizowane.
		\end{enumerate}
...
- definicje pakietów usług
	- lista konsultantów
	- obsługiwane kody badania
	- czas na zrealizowanie zlecenia
	- czy zleceniodawca może wybrać preferowanego konsultanta ???
	- czy zlecenie ma się pojawić u wszystkich konsultanów 
- mapowanie tagów DICOM na kody badania
...
Działanie systemu MUSI uwzględniać obsługę niżej opisanych schematów dziłania.
...
\begin{enumerate}
  ...
  TODO: co jeśli pacjenta nie ma w PACS'ie? co jeśli $L_S$ jest połączony z więcej niż jednym LSS - węzeł lokalny swój własny id pacjenta i wiąże go z LSS_ID 
